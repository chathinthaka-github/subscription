...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($msisdn) || empty($shortcode) || empty($keyword)) {
        ...
        // Debug info
        $debugInfo = "HTTP Code: {$httpCode}<br>";
        $debugInfo .= "Request Data: " . json_encode($data) . "<br>";
        $debugInfo .= "Response: " . htmlspecialchars($response) . "<br>";
        if ($curlError) {
            $debugInfo .= "cURL Error: " . htmlspecialchars($curlError) . "<br>";
        }
...
<head>
    ...
    <title>Test Subscribe - Debug Mode</title>
    <style>
        * {
            margin: 0;
            padding: 0;
...
<body>
    <div class="container">
        <h1>🔧 Test Subscribe (Debug Mode)</h1>
        ...
        <a href="subscribe-page.php" class="back-link">← Back to Normal Subscribe Page</a>
    </div>
