...
<!-- Header -->
<div class="mb-8">
    <h1 class="text-2xl font-bold text-gray-900">Create Renewal Plan</h1>
    <p class="mt-1 text-sm text-gray-500">Configure a new renewal schedule for a service</p>
</div>
    ...
    <div class="mb-6 p-4 bg-amber-50 border border-amber-200 rounded-lg">
        <div class="flex items-start gap-3">
            ...
            <div>
                ...
                <p class="text-sm text-amber-700 mt-1">Each service can only have one renewal plan. Creating a new plan for an existing service will replace the current plan.</p>
            </div>
        </div>
    </div>
    ...
    <form method="POST" action="{{ route('renewal-plans.store') }}" id="planForm">
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6">
            ...
            <div>
                ...
                <input type="text" name="name" id="name" required value="{{ old('name') }}" maxlength="100" class="neo-input" placeholder="e.g., Daily Subscription">
                @error('name')
                    <p class="mt-1 text-sm text-red-500">{{ $message }}</p>
                @enderror
            </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6">
            <div>
                ...
                <input type="text" name="price_code" id="price_code" value="{{ old('price_code') }}" maxlength="50" class="neo-input" placeholder="e.g., PRICE100">
            ...
            <div class="flex flex-col gap-4">
                <label class="flex items-center gap-3 cursor-pointer">
                    <input type="checkbox" name="skip_subscription_day" id="skip_subscription_day" value="1" {{ old('skip_subscription_day') ? 'checked' : '' }} class="w-4 h-4 rounded border-gray-300 text-blue-600 focus:ring-blue-500">
                    <span class="text-sm text-gray-700">Skip Subscription Day</span>
                </label>
                <label class="flex items-center gap-3 cursor-pointer">
                    <input type="checkbox" name="is_fixed_time" id="is_fixed_time" value="1" {{ old('is_fixed_time') ? 'checked' : '' }} class="w-4 h-4 rounded border-gray-300 text-blue-600 focus:ring-blue-500">
        ...
        <div id="fixed-time" style="display: none;" class="mb-6">
            ...
            <input type="time" name="fixed_time" id="fixed_time_input" value="{{ old('fixed_time') }}" class="neo-input max-w-xs">
        ...
        <div id="weekly-days" style="display: none;" class="mb-6">
            ...
            <div class="grid grid-cols-2 sm:grid-cols-4 md:grid-cols-7 gap-3">
                ...
                <label class="flex items-center gap-2 p-3 bg-gray-50 rounded-lg cursor-pointer hover:bg-blue-50 transition-colors border border-gray-200 has-[:checked]:bg-blue-500 has-[:checked]:border-blue-500 has-[:checked]:text-white">
                    <input type="checkbox" name="schedule_rules[days][]" value="{{ $value }}" class="w-4 h-4 rounded border-gray-300 text-blue-600 focus:ring-blue-500">
        ...
        <div id="monthly-days" style="display: none;" class="mb-6">
            ...
            <div class="grid grid-cols-7 sm:grid-cols-10 md:grid-cols-15 gap-2">
                ...
                <label class="relative flex items-center justify-center p-3 bg-gray-50 rounded-lg cursor-pointer hover:bg-blue-50 transition-colors border border-gray-200 has-[:checked]:bg-blue-500 has-[:checked]:border-blue-500 has-[:checked]:text-white">
                    <input type="checkbox" name="schedule_rules[days][]" value="{{ $i }}" class="absolute opacity-0 w-full h-full cursor-pointer">
