...
<div class="neo-card p-6 rounded-2xl max-w-2xl">
    <h1 class="text-2xl font-semibold mb-6" style="color: var(--color-neo-text);">Edit Service</h1>
    ...
    <form method="POST" action="{{ route('services.update', $service) }}" class="space-y-6">
        ...
        <div>
            ...
            <select name="shortcode_id" id="shortcode_id" required class="neo-input px-4 py-2 w-full" style="color: var(--color-neo-text);">
                ...
                @foreach($shortcodes as $shortcode)
                    <option value="{{ $shortcode->id }}" {{ old('shortcode_id', $service->shortcode_id) == $shortcode->id ? 'selected' : '' }}>
        ...
        <div>
            ...
            <input type="text" name="keyword" id="keyword" required value="{{ old('keyword', $service->keyword) }}" 
        ...
        <div>
            <div class="flex items-center">
                <input type="checkbox" name="fpmt_enabled" id="fpmt_enabled" value="1" {{ old('fpmt_enabled', $service->fpmt_enabled) ? 'checked' : '' }}
