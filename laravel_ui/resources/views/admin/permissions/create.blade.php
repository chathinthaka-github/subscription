...
<div class="px-4 sm:px-6 lg:px-8">
    ...
    <form method="POST" action="{{ route('admin.permissions.store') }}" class="mt-8 space-y-6 max-w-2xl" id="permissionForm">
        @csrf
        ...
        <div>
            ...
            <select name="module" id="module" required class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm">
                <option value="service" {{ old('module') === 'service' ? 'selected' : '' }}>Service</option>
                <option value="service_message" {{ old('module') === 'service_message' ? 'selected' : '' }}>Service Message</option>
                <option value="renewal_plan" {{ old('module') === 'renewal_plan' ? 'selected' : '' }}>Renewal Plan</option>
        ...
        <div>
            ...
            <input type="text" name="name" id="name" readonly value="{{ old('name') }}" class="mt-1 block w-full rounded-md border-gray-300 bg-gray-50 shadow-sm sm:text-sm">
        </div>
