...
<div class="neo-card p-6 rounded-2xl max-w-2xl">
    <h1 class="text-2xl font-semibold mb-6" style="color: var(--color-neo-text);">Edit Shortcode</h1>

    <form method="POST" action="{{ route('shortcodes.update', $shortcode) }}" class="space-y-6">
        @csrf
        @method('PUT')

        <div>
            <label for="shortcode" class="block text-sm font-medium mb-2" style="color: var(--color-neo-text);">Shortcode</label>
            <input type="text" name="shortcode" id="shortcode" required value="{{ old('shortcode', $shortcode->shortcode) }}" 
        ...
        <div>
            ...
            <textarea name="description" id="description" rows="3" 
                      class="neo-input px-4 py-2 w-full" style="color: var(--color-neo-text);" 
                      placeholder="Optional description">{{ old('description', $shortcode->description) }}</textarea>
            @error('description')
                <p class="mt-1 text-sm" style="color: var(--color-neo-error);">{{ $message }}</p>
            @enderror
        </div>
        ...
        <div>
            ...
            <select name="status" id="status" required class="neo-input px-4 py-2 w-full" style="color: var(--color-neo-text);">
                <option value="active" {{ old('status', $shortcode->status) === 'active' ? 'selected' : '' }}>Active</option>
                <option value="inactive" {{ old('status', $shortcode->status) === 'inactive' ? 'selected' : '' }}>Inactive</option>
            </select>
            @error('status')
                <p class="mt-1 text-sm" style="color: var(--color-neo-error);">{{ $message }}</p>
            @enderror
        ...
        <div class="flex gap-4">
            <button type="submit" class="neo-button-primary px-6 py-2 text-sm font-medium">Update</button>
